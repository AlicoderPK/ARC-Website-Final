...
import { ArrowRight, CheckCircle, Users, Award, Clock, Star, Calculator, FileText, Building, TrendingUp, MessageCircle, Phone, X, Info, LucideIcon } from 'lucide-react';
...
const Home = () => {
  ...
  const transitionToService = (newIndex: number) => {
  ...
  const nextService = () => {
    const nextIndex = (currentServiceIndex + 1) % allServices.length;
    transitionToService(nextIndex);
  };

  const prevService = () => {
    const prevIndex = (currentServiceIndex - 1 + allServices.length) % allServices.length;
    transitionToService(prevIndex);
  };
  ...
  const getDisplayServices = () => {
    ...
    return [
      { ...allServices[prevIndex], position: 'prev' as const },
      { ...allServices[currentServiceIndex], position: 'current' as const },
      { ...allServices[nextIndex], position: 'next' as const }
    ];
  };
  ...
  return (
    <div>
      <style>{`
        ...
        .service-card {
          transition: opacity 0.3s ease-in-out, scale 0.3s ease-in-out, width 0.3s ease-in-out;
        }
      `}</style>
      ...
      <section id="services" className="py-20 bg-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            ...
            <button
              onClick={prevService}
              className="absolute left-4 top-1/2 transform -translate-y-1/2 z-20 bg-white/90 hover:bg-white text-gray-700 p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300"
            >
              <ArrowRight className="h-6 w-6 rotate-180" />
            </button>
            <button
              onClick={nextService}
              className="absolute right-4 top-1/2 transform -translate-y-1/2 z-20 bg-white/90 hover:bg-white text-gray-700 p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300"
            >
              ...
              {getDisplayServices().map((service, index) => {
                ...
                return (
                  <div
                    ...
                    className={`service-card ${isCenter ? 'w-96 scale-110 opacity-100' : 'w-80 scale-90 opacity-70'}`}
                  >
                    <div 
                      className={`bg-white p-8 rounded-2xl shadow-lg relative transition-transform duration-300 transform hover:scale-105 hover:shadow-2xl ${
                        isCenter ? 'border-4 border-blue-600' : 'border border-blue-200'
                      }`}
                    >
                      {showBadge && (
                        <div 
                          className={`absolute -top-3 left-1/2 transform -translate-x-1/2 transition-opacity duration-300 ease-in-out ${
                            isCenter && badgeVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-90 pointer-events-none'
                          }`}
                        >
